...
namespace FoodTech.ParseString;
    ...
    IEquatable<Guid>
{
    ...
    public static Uuid7 FromString(string input) {
        return Parse(input, provider: null);
    }

    public static Uuid7 Parse(string input, IFormatProvider provider)
    {
        if (string.IsNullOrEmpty(input)) ThrowHelper.ThrowCannotBeNullOrEmpty();
        if (TryParse(input, provider, out var result))
        {
            return result;
        }

        ThrowHelper.ThrowInvalidUuidFormatSpecification();
        throw new Exception();
    }

    public static bool TryParse(string input, IFormatProvider provider, out Uuid7 result)
    {
        var charArray = input.AsSpan().Trim();
        result = new Uuid7();
        return true;
    }
